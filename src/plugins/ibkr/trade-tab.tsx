...
export function TradeTab({ focused, width, onCapture }: DetailTabProps) {
  ...
  usePaneFooter("ibkr-trade", () => ({
    ...
    hints: [
      ...
      { id: "instrument", key: "s", label: "ymbol", onPress: () => chooseInstrument().catch(() => {}), disabled: ticketState.busy },
      { id: "account", key: "a", label: "ccount", onPress: () => chooseAccount().catch(() => {}), disabled: ticketState.busy },
      ...
      { id: "quantity", key: "q", label: "ty", onPress: () => editNumericField("Quantity", ticketState.draft.quantity, (value) => { if (value != null && symbol && ticker) setTradeTicketDraft(symbol, { quantity: value }, ticker); }).catch(() => {}), disabled: ticketState.busy || !symbol || !ticker },
      { id: "type", key: "t", label: "ype", onPress: () => editOrderType().catch(() => {}), disabled: ticketState.busy },
      { id: "limit", key: "l", label: "imit", onPress: () => editPriceField("Limit Price", ticketState.draft.limitPrice, (value) => { if (symbol && ticker) setTradeTicketDraft(symbol, { limitPrice: value }, ticker); }).catch(() => {}), disabled: ticketState.busy || !showLimit || !symbol || !ticker },
      ...
      { id: "preview", key: "p", label: "review", onPress: () => previewOrder().catch(() => {}), disabled: ticketState.busy || !symbol || !ticker },
    ],
  }), [
    chooseAccount,
