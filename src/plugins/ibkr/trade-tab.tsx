...
export function TradeTab({ focused, width, onCapture }: DetailTabProps) {
  ...
  usePaneFooter("ibkr-trade", () => ({
    ...
    hints: [
      ...(!ticketState.busy ? [
        { id: "refresh", key: "r", label: "efresh", onPress: () => refresh().catch(() => {}) },
        { id: "profile", key: "i", label: "profile", onPress: () => chooseBrokerInstance().catch(() => {}) },
        { id: "account", key: "a", label: "ccount", onPress: () => chooseAccount().catch(() => {}) },
      ] : []),
      ...(!ticketState.busy && symbol && ticker ? [
        { id: "side", key: "b/v", label: "side", onPress: () => setTradeTicketDraft(symbol, { action: ticketState.draft.action === "BUY" ? "SELL" : "BUY" }, ticker) },
        { id: "quantity", key: "q", label: "ty", onPress: () => editNumericField("Quantity", ticketState.draft.quantity, (value) => { if (value != null) setTradeTicketDraft(symbol, { quantity: value }, ticker); }).catch(() => {}) },
        { id: "type", key: "t", label: "ype", onPress: () => editOrderType().catch(() => {}) },
        ...(showLimit ? [{ id: "limit", key: "l", label: "imit", onPress: () => editPriceField("Limit Price", ticketState.draft.limitPrice, (value) => { setTradeTicketDraft(symbol, { limitPrice: value }, ticker); }).catch(() => {}) }] : []),
        ...(showStop ? [{ id: "stop", key: "x", label: "stop", onPress: () => editPriceField("Stop Price", ticketState.draft.stopPrice, (value) => { setTradeTicketDraft(symbol, { stopPrice: value }, ticker); }).catch(() => {}) }] : []),
        { id: "preview", key: "p", label: "review", onPress: () => previewOrder().catch(() => {}) },
      ] : []),
    ],
  }), [
    chooseAccount,
