...
describe("Bloomberg function panes", () => {
  ...
  test("defaults GR to SPY when one ticker is provided", async () => {
    const template = bloombergFunctionsPlugin.paneTemplates?.find((entry) => entry.id === "relationship-graph-pane");
    const context = {
      config: createDefaultConfig("/tmp/gloomberb-bloomberg-functions"),
