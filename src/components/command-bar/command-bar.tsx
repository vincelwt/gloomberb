...
function isRouteCommandId(commandId: string): boolean {
...
function getVisibleWorkflowFields(
...
function coerceFieldString(value: CommandBarFieldValue | undefined): string {
  return typeof value === "string" ? value : "";
}

function coerceFieldBoolean(value: CommandBarFieldValue | undefined): boolean {
  return value === true;
}

function coerceFieldValues(value: CommandBarFieldValue | undefined): string[] {
...
function summarizeWorkflowFieldValue(field: CommandBarWorkflowField, value: CommandBarFieldValue | undefined): string {
  switch (field.type) {
    case "toggle":
...
function summarizePaneSettingValue(field: PaneSettingField, value: unknown): string {
  switch (field.type) {
    case "toggle":
...
function normalizeWizardFields(steps: WizardStep[]): {
  fields: CommandBarWorkflowField[];
  description: string[];
...
function routeCommandIdToScreen(commandId: string): "ticker-search" | "themes" | "plugins" | "layout" | "new-pane" | null {
  switch (commandId) {
    case "search-ticker":
...
function getFirstVisibleFieldId(
  fields: CommandBarWorkflowField[],
  values: Record<string, CommandBarFieldValue>,
): string | null {
...
function isWorkflowTextField(field: CommandBarWorkflowField | undefined): boolean {
  return field?.type === "text" || field?.type === "password" || field?.type === "number";
}

function buildGeneratedTemplateField(
...
function getScreenFooterLeft(route: CommandBarRoute | null): string {
...
function getScreenFooterRight(route: CommandBarRoute | null): string {
...
function isRootParsedCommand(commandId: string): boolean {
  return commandId === "search-ticker"
...
function isCollectionCommand(commandId: string): commandId is "add-watchlist" | "add-portfolio" | "remove-watchlist" | "remove-portfolio" {
...
function getCollectionCommandKind(commandId: "add-watchlist" | "add-portfolio" | "remove-watchlist" | "remove-portfolio"): CollectionKind {
  return commandId === "add-watchlist" || commandId === "remove-watchlist" ? "watchlist" : "portfolio";
}

function getCollectionCommandAction(commandId: "add-watchlist" | "add-portfolio" | "remove-watchlist" | "remove-portfolio"): CollectionMembershipAction {
  return commandId === "add-watchlist" || commandId === "add-portfolio" ? "add" : "remove";
}

function getCollectionCommandVerb(action: CollectionMembershipAction): string {
  return action === "add" ? "Add" : "Remove";
}

function looksDestructiveCommand(command: Pick<CommandDef, "id" | "label" | "description" | "keywords">): boolean {
  const haystack = [
    command.id,
    command.label,
    command.description || "",
